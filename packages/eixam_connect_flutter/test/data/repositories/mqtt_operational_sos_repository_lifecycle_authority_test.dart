...
void main() {
  group('MqttOperationalSosRepository lifecycle authority', () {
    ...
    setUp(() {
      ...
      repository = MqttOperationalSosRepository(realtimeClient: realtimeClient);
    });
    ...
    test(
      'clears stale restored open state without incident and publishes SOS',
      () async {
        ...
        expect(
          _hasDiagnostic('SOS_ALREADY_ACTIVE_GUARD_STALE_CLEARED'),
          isTrue,
        );
        expect(
          _hasDiagnostic('SOS_TRIGGER_CONTINUES_AFTER_STALE_GUARD_CLEAR'),
          isTrue,
        );
      },
    );
    ...
    test(
      'keeps duplicate protection when backend rehydrates active SOS',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource()
    ...
    test(
      ...
      () async {
        ...
        await repository.submitSosToBackend(
          ...
          relayHardwareId: 'relay-hardware',
          relaySource: 'remote_lora_relay',
          cycleKey: 'remote:$originatorNodeId:8',
        );
        realtimeClient.emitEvent(
          _processedEvent(
            incidentId: 'remote-canonical-incident-next',
            timestamp: nextOccurredAt,
          ),
        );
        await _pumpRealtime();
    ...
    test(
      'user-scoped portal ACK confirms reception without clientIncidentId',
      () async {
        final local = await _triggerAppSos(repository);
        expect(local.isBackendConfirmed, isFalse);

        realtimeClient.emitEvent(
          _portalAckEvent(incidentId: 'backend-ack-incident'),
        );
        await _pumpRealtime();
        ...
        expect(
          current.progress.steps.single.type,
    ...
    test(
      'remote canonical admission rejects a different publish timestamp',
      () async {
        const originatorNodeId = 0x01020304;
        final occurredAt = DateTime.utc(2026, 9, 24, 11);
        await repository.submitSosToBackend(
          timestamp: occurredAt,
          positionSnapshot: null,
          deviceId: originatorNodeId.toString(),
        ...
        realtimeClient.emitEvent(
          _processedEvent(
            incidentId: 'wrong-remote-canonical',
            timestamp: occurredAt.add(const Duration(milliseconds: 1)),
          ),
        );
        await _pumpRealtime();
    ...
    test(
      ...
      () async {
        const originatorNodeId = 0x01020304;
        const relayNodeId = 0x05060708;
        const incidentId = 'restored-remote-canonical';
        final occurredAt = DateTime.utc(2026, 9, 24, 12);
        final store = MemorySharedPrefsSdkStore();
        final remoteDataSource = _FakeSosRemoteDataSource()
          ..active = SosIncidentDto(
            id: incidentId,
            ...
            relayNodeId: relayNodeId,
            deviceId: originatorNodeId.toString(),
            cycleKey: 'remote:$originatorNodeId:9',
            originKind: SosOriginKind.remoteRelay.name,
            actionability: SosActionability.externalOnly.name,
            displaySurface: SosDisplaySurface.activeAndHistory.name,
          );
    ...
    test(
      'internal-topic portal ACK without payload userId still confirms',
      () async {
        await _triggerAppSos(repository);
    ...
    test('accepts matching active incidentId lifecycle update', () async {
      ...
      realtimeClient.emitEvent(
        _lifecycleEvent(incidentId: incident.id, state: 'acknowledged'),
      );
      await _pumpRealtime();
    ...
    test('accepts matching correlationId lifecycle update', () async {
      ...
      expect(
        _hasDiagnostic('MQTT_SOS_LIFECYCLE_ACCEPTED_CORRELATION_MATCH'),
        isTrue,
      );
    });
    ...
    test(
      'settles active to cancelled through validated transition path',
      () async {
        final incident = await _triggerAppSos(repository);
    ...
    test(
      ...
      () async {
        ...
        realtimeClient.emitEvent(
          _lifecycleEvent(incidentId: incident.id, state: 'resolved'),
        );
        await _pumpRealtime();
    ...
    test(
      ...
      () async {
        ...
        final sub = repository.watchRejectedTerminalReconciliations().listen(
          requests.add,
        );
    ...
    test(
      'accepts legacy snake-case terminal incident ID as a fallback',
      () async {
        final incident = await _triggerAppSos(repository);
        final requests = <SosRejectedTerminalReconciliationRequest>[];
        final sub = repository.watchRejectedTerminalReconciliations().listen(
    ...
    test(
      'cold restore can seed terminal state without live validation',
      () async {
        final store = MemorySharedPrefsSdkStore()
          ..stringValues[SharedPrefsSdkStore.sosStateKey] =
    ...
    test(
      'rejects mismatched incidentId without correlation or cycle proof',
      () async {
        final incident = await _triggerAppSos(repository);

        realtimeClient.emitEvent(
          _lifecycleEvent(incidentId: 'other-incident', state: 'cancelled'),
        );
        await _pumpRealtime();
        ...
        expect(await repository.getSosState(), SosState.sent);
        expect(
          _hasDiagnostic('MQTT_SOS_LIFECYCLE_REJECTED_IDENTITY_MISMATCH'),
          isTrue,
        );
      },
    );
    ...
    test('rejects stale terminal replay after newer active incident', () async {
      ...
      realtimeClient.emitEvent(
        _lifecycleEvent(incidentId: staleIncident.id, state: 'cancelled'),
      );
      await _pumpRealtime();
      expect(await repository.getSosState(), SosState.cancelled);
      ...
      realtimeClient.emitEvent(
        _lifecycleEvent(incidentId: staleIncident.id, state: 'resolved'),
      );
      await _pumpRealtime();
    ...
    test(
      'rejects external remote_lora_relay lifecycle for local active SOS',
      () async {
        final incident = await _triggerAppSos(repository);
        ...
        expect(await repository.getSosState(), SosState.sent);
        expect(
          _hasDiagnostic('MQTT_SOS_LIFECYCLE_REJECTED_EXTERNAL_ONLY'),
          isTrue,
        );
      },
    );
    ...
    test(
      'leaves external remote_lora_relay lifecycle history-only when idle',
      () async {
        realtimeClient.emitEvent(
          _lifecycleEvent(
    ...
    test('accepts actuator_update for matching active incident', () async {
      ...
      realtimeClient.emitEvent(
        _actuatorEvent(incidentId: incident.id, snapshotVersion: 7),
      );
      await _pumpRealtime();
    ...
    test('older and equal actuator versions cannot regress delivery', () async {
      ...
      expect(current!.actuators!.snapshotVersion, 7);
      expect(
        current.actuators!.items.single.status,
        SosActuatorStatus.delivered,
      );
    });

    test(
      'buffers a canonical actuator candidate while handoff is pending',
      () async {
        await _triggerAppSos(repository);

        realtimeClient.emitEvent(
          _actuatorEvent(incidentId: 'other-incident', snapshotVersion: 7),
        );
        await _pumpRealtime();
    ...
    test(
      'processed lifecycle confirms reception before actuator planning',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource();
        await repository.dispose();
        repository = MqttOperationalSosRepository(
    ...
    test(
      ...
      () async {
        ...
        expect(current?.isBackendConfirmed, isTrue);
        expect(
          _hasDiagnostic('reason=processed_publish_timestamp_match'),
          isTrue,
        );
    ...
    test(
      'processed event without correlation cannot claim local SOS',
      () async {
        final localIncident = await _triggerAppSos(repository);

        realtimeClient.emitEvent(
          _processedEvent(incidentId: 'canonical-processed-incident'),
        );
        await _pumpRealtime();
    ...
    test(
      'publish stays local and never calls REST before processed event',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource();
        await repository.dispose();
        repository = MqttOperationalSosRepository(
    ...
    test(
      'processed plan and later actuator update expose two contacts',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource();
        await repository.dispose();
        repository = MqttOperationalSosRepository(
    ...
    test(
      'MQTT contact delivery plus failure exposes partial progress',
      () async {
        final local = await _triggerAppSos(repository);
        realtimeClient.emitEvent(
          _processedEvent(
        ...
        final contacts = (await repository.getCurrentIncident())!.progress.steps
            .singleWhere(
              (step) => step.type == SosProgressStepType.emergencyContacts,
            );
        expect(contacts.totalTargets, 2);
    ...
    test(
      'MQTT warning timeout reconciles an active backend incident',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource();
        await repository.dispose();
        repository = MqttOperationalSosRepository(
    ...
    test(
      ...
      () async {
        ...
        await _pumpRealtime();
        expect(
          (await repository.getCurrentIncident())?.isBackendConfirmed,
          isFalse,
        );
    ...
    test(
      'actuator-assisted REST fallback rejects conflicting device ownership',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource();
        await repository.dispose();
        repository = MqttOperationalSosRepository(
    ...
    test(
      'device-origin provisional id hands off to backend confirmation',
      () async {
        ...
        expect(current.isBackendConfirmed, isTrue);
        expect(
          current.progress.provisionalIncidentId,
          'device-runtime-13579:7',
        );
    ...
    test(
      'null cancel response stays pending while backend remains active',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource()
    ...
    test(
      'active cancel response stays pending until terminal MQTT evidence',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource()
    ...
    test(
      'HTTP failure of provisional sos-* settles cancelled locally',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource()
    ...
    test(
      'missing cancel HTTP still settles provisional sos-* locally',
      () async {
        final local = await _triggerAppSos(repository);
    ...
    test(
      'wrong incident terminal response cannot close cancellation',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource();
        await repository.dispose();
        repository = MqttOperationalSosRepository(
    ...
    test(
      'timeout result from a stale lifecycle generation is ignored',
      () async {
        final query = Completer<SosIncidentDto?>();
        final remoteDataSource = _FakeSosRemoteDataSource()
          ..getActiveSosCompleter = query;
    ...
    test(
      'cancel during confirmation stops retries and cannot reopen',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource();
        await repository.dispose();
        repository = MqttOperationalSosRepository(
        ...
        await _triggerAppSos(repository);
        realtimeClient.emitEvent(
          _actuatorEvent(incidentId: 'late-active', snapshotVersion: 2),
        );
        await _pumpRealtime();
    ...
    test(
      'session clear stops confirmation and removes local progress',
      () async {
        final remoteDataSource = _FakeSosRemoteDataSource();
        await repository.dispose();
        repository = MqttOperationalSosRepository(
        ...
        await _triggerAppSos(repository);
        realtimeClient.emitEvent(
          _actuatorEvent(incidentId: 'other-user-incident', snapshotVersion: 2),
        );
        await _pumpRealtime();
        await repository.clearSosRuntimeForSessionChange();
        realtimeClient.emitEvent(
    ...
    test(
      'stale actuator event cannot reopen a locally cancelled incident',
      () async {
        await repository.dispose();
        repository = MqttOperationalSosRepository(
          realtimeClient: realtimeClient,
        ...
        realtimeClient.emitEvent(
          _actuatorEvent(incidentId: incident.id, snapshotVersion: 99),
        );
        await _pumpRealtime();
    ...
    test(
      'cached confirmed active incident clears on no-active REST response',
      () async {
        final store = MemorySharedPrefsSdkStore();
        ...
        store.stringValues[SharedPrefsSdkStore.sosStateKey] =
            SosState.sent.name;
        await repository.dispose();
        repository = MqttOperationalSosRepository(
          realtimeClient: realtimeClient,
...
Future<SosIncident> _triggerAppSos(
  ...
  return repository.triggerSos(triggerSource: 'button_ui', cycleKey: cycleKey);
}
