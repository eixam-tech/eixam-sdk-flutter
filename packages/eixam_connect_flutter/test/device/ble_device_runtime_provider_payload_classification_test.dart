...
void main() {
  group('BleDeviceRuntimeProvider payload classification', () {
    ...
    test(
      'classifies a 12-byte TEL notify as SOS when SOS decode is valid',
      () async {
    ...
    test(
      ...
      () async {
        ...
        final event = await nextEvent;
        final status = await nextSosStatus.timeout(const Duration(seconds: 2));
        expect(event.type, BleIncomingEventType.sosMeshPacket);
        expect(event.classification.kind, BleIncomingPayloadKind.ownDeviceSos);
    ...
    test(
      ...
      () async {
        ...
        expect(
          event.remoteRelaySosSnapshot!.location!.longitude,
    ...
    test(
      'classifies a 12-byte SOS notify as remote relay with packet location',
      () async {
        await runtimeProvider.requestDeviceRuntimeStatus();
        final nextEvent = _nextIncomingEvent(runtimeProvider);
    ...
    test(
      ...
      () async {
        ...
        expect(event.remoteRelaySosSnapshot, isNotNull);
        expect(
          event.remoteRelaySosSnapshot!.source,
          RemoteRelaySosSource.d2Relay,
        );
        expect(event.remoteRelaySosSnapshot!.originatorNodeId, 0x12345678);
        expect(event.remoteRelaySosSnapshot!.relayNodeId, 0x1234);
        expect(event.remoteRelaySosSnapshot!.location, isNotNull);
    ...
    test(
      'classifies 6-byte 0xDA as nearby TX status, not an SOS event',
      () async {
        await runtimeProvider.requestDeviceRuntimeStatus();
        final nextEvent = _nextIncomingEvent(runtimeProvider);
        ...
        expect(event.sosEventPacket, isNull);
        expect(
          (await runtimeProvider.deviceSosController.getStatus()).state,
          DeviceSosState.inactive,
        );
    ...
    test('reassembles 0xD8 nearby RX from 0xD0 fragments', () async {
      const blob = <int>[
        0xD8,
        0x78,
        0x56,
      ...
      final nextEvent = runtimeProvider.watchIncomingEvents().firstWhere(
        (event) => event.type == BleIncomingEventType.nearbyTextRx,
      );

      bleClient.emitNotification(
        MockBleClient.demoDeviceId,
        channel: EixamBleChannel.tel,
        payload: <int>[0xD0, 24, 0, 0, 0, ...blob.sublist(0, 15)],
      );
      bleClient.emitNotification(
        MockBleClient.demoDeviceId,
        channel: EixamBleChannel.tel,
        payload: <int>[0xD0, 24, 0, 15, 0, ...blob.sublist(15)],
      );

      final event = await nextEvent.timeout(const Duration(seconds: 2));
      expect(event.nearbyTextPacket?.text, 'ok');
      expect(event.nearbyTextPacket?.fromNodeId, 0x12345678);
      expect(event.nearbyTextPacket?.destNodeId, 0xFFFFFFFF);
      expect(event.aggregatePayload, blob);
    });

    test(
      'applies a TEL notify E1 02 for the connected node as a local cancel',
      () async {
        await runtimeProvider.requestDeviceRuntimeStatus();
        final activated = runtimeProvider.deviceSosController
            .watchStatus()
            .firstWhere((status) => status.state != DeviceSosState.inactive);
        bleClient.emitNotification(
          MockBleClient.demoDeviceId,
          channel: EixamBleChannel.tel,
          payload: const <int>[
    ...
    test(
      'classifies a 12-byte TEL notify as TEL when SOS decode is not valid',
      () async {
        final nextEvent = _nextIncomingEvent(runtimeProvider);
    ...
    test(
      'classifies a port 260 12-byte TEL notify as cluster heartbeat',
      () async {
        final nextEvent = _nextIncomingEvent(runtimeProvider);
    ...
    test(
      'classifies direct and D0-reassembled D1 before generic TEL fallback',
      () async {
        ...
        bleClient.emitNotification(
          MockBleClient.demoDeviceId,
          channel: EixamBleChannel.tel,
          payload: const <int>[0xD1, 0x01, 7, 1, 1, 0, 4, 0, 0, 0, 4, 0, 0, 0],
        );
        ...
        const payload = <int>[0xD1, 0x01, 8, 1, 1, 0, 5, 0, 0, 0, 5, 0, 0, 0];
        bleClient.emitNotification(
          MockBleClient.demoDeviceId,
          channel: EixamBleChannel.tel,
    ...
    test('routes E9 7A, E9 78, D3, TEL and SOS without collisions', () async {
      ...
      bleClient.emitNotification(
        MockBleClient.demoDeviceId,
        channel: EixamBleChannel.tel,
        payload: const <int>[0x34, 0x12, 0, 0, 1, 2, 3, 4, 5, 6, 0x80, 0x25],
      );
      bleClient.emitNotification(
        MockBleClient.demoDeviceId,
        channel: EixamBleChannel.sos,
      ...
      expect(
        routed.first.provisioningCommandResult?.outcome,
        ProvisioningCommandOutcome.ok,
      );
    });

    test(
      'does not swallow a 7-byte SOS whose node id starts with D3',
      () async {
        final nextEvent = runtimeProvider.watchIncomingEvents().firstWhere(
          (event) => event.type == BleIncomingEventType.sosMeshPacket,
        );
    ...
    test(
      'does not swallow a 12-byte SOS whose node id starts with D3',
      () async {
        final nextEvent = runtimeProvider.watchIncomingEvents().firstWhere(
          (event) => event.type == BleIncomingEventType.sosMeshPacket,
        );
    ...
    test(
      'does not swallow a classic TEL whose node id starts with D3',
      () async {
        final nextEvent = runtimeProvider.watchIncomingEvents().firstWhere(
    ...
    test(
      'malformed D3-shaped data falls through to unknown classification',
      () async {
        final nextEvent = _nextIncomingEvent(runtimeProvider);
    ...
    test(
      'reassembles a maximum 386-byte D3 payload into one typed batch',
      () async {
