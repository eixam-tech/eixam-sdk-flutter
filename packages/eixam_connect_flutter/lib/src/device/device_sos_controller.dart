...
class DeviceSosController {
  ...
  Future<void> _dispatchCommand({
    ...
    try {
      ...
      BleDebugRegistry.instance.recordEvent(
        'SOS_DEVICE_COMMAND_TRANSPORT_DISPATCH '
        'route=$commandRouteLabel command=${command.label} '
        'opcode=0x${command.opcode.toRadixString(16).padLeft(2, '0')} '
        'characteristic=${command.targetCharacteristicUuid}',
      );
  ...
  DeviceSosStatus _queueTerminalCommand({
    ...
    if (!_terminalOperationIsCurrent(operationIsCurrent)) {
      _recordStaleTerminalOperation(commandRouteLabel ?? 'attached_writer');
      return _status;
    }
    final now = _now();
  ...
  void _ensureCommandAvailable({required EixamDeviceCommand command}) {
    final available = command.usesCmdCharacteristic
        ? longCommandAvailable ||
              (command.supportsLegacyInetFallback && shortCommandAvailable)
    ...
    final readiness = command.usesCmdCharacteristic
        ? 'longCommandAvailable=$longCommandAvailable '
              'legacyShortFallback=${command.supportsLegacyInetFallback && shortCommandAvailable}'
  ...
  void handleIncomingSosPacket(
    ...
    if (nextState == DeviceSosState.preConfirm) {
      ...
      _enterPreConfirm(
        ...
        packetId: packet.packetId,
        hasLocation: packet.hasValidPosition
            ? true
