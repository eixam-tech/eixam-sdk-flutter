...
class EixamDeviceCommand {
  ...
  factory EixamDeviceCommand.sosCancel({bool forceCmdCharacteristic = false}) =>
  ...
  factory EixamDeviceCommand.nearbyTextTxFragment(List<int> frame) {
    if (frame.isEmpty ||
        frame.first != EixamBleProtocol.nearbyTextTxOpcode ||
        frame.length > 20) {
      throw ArgumentError('Invalid nearby text TX fragment.');
    }
    return EixamDeviceCommand._(
      opcode: EixamBleProtocol.nearbyTextTxOpcode,
      label: 'NEARBY TEXT TX',
      bytes: List<int>.unmodifiable(frame),
      forceCmdCharacteristic: true,
    );
  }

  factory EixamDeviceCommand.nearbyOwnerNameFragment(List<int> frame) {
    if (frame.isEmpty ||
        frame.first != EixamBleProtocol.nearbyOwnerNameOpcode ||
        frame.length > 20) {
      throw ArgumentError('Invalid nearby owner name fragment.');
    }
    return EixamDeviceCommand._(
      opcode: EixamBleProtocol.nearbyOwnerNameOpcode,
      label: 'NEARBY OWNER NAME',
      bytes: List<int>.unmodifiable(frame),
      forceCmdCharacteristic: true,
    );
  }

  factory EixamDeviceCommand.nearbyGroupFragment(List<int> frame) {
    if (frame.isEmpty ||
        frame.first != EixamBleProtocol.nearbyTextGroupOpcode ||
        frame.length > 20) {
      throw ArgumentError('Invalid nearby group fragment.');
    }
    return EixamDeviceCommand._(
  ...
  BleCommandCriticality get criticality {
    return switch (opcode) {
      ...
      0x22 ||
      0x24 ||
      0x25 => BleCommandCriticality.critical,
