...
Short example for every current public method on `EixamConnectSdk`, including methods excluded from the partner site.
...
**Signature**: `Future<GuidedRescueState> getGuidedRescueState()`
...
**Signature**: `Stream<GuidedRescueState> watchGuidedRescueState()`

```dart
sdk.watchGuidedRescueState().listen((state) { /* internal validation */ });
```

### `setGuidedRescueSession`

**Signature**: `Future<GuidedRescueState> setGuidedRescueSession({ required int targetNodeId, required int rescueNodeId, })`

```dart
await sdk.setGuidedRescueSession(targetNodeId: 1001, rescueNodeId: 2001);
```

### `clearGuidedRescueSession`

**Signature**: `Future<void> clearGuidedRescueSession()`
...
**Signature**: `Future<void> requestGuidedRescuePosition()`
...
**Signature**: `Future<void> acknowledgeGuidedRescueSos()`
...
**Signature**: `Future<void> enableGuidedRescueBuzzer()`
...
**Signature**: `Future<void> disableGuidedRescueBuzzer()`
...
**Signature**: `Future<void> requestGuidedRescueStatus()`
