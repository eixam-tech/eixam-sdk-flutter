...
Checkpoint status: local storage is partially mitigated, not fully fixed. Secure storage migration is designed/planned in `docs/audit/secure-storage-abstraction-plan.md`; Phase 1 now provides a pure Dart SDK core abstraction, but no secure-storage dependency, platform-backed implementation, runtime migration, or user-data migration exists yet in this SDK/app boundary.
...
- Secure storage migration is designed/planned in `docs/audit/secure-storage-abstraction-plan.md` but not fixed in this checkpoint: neither app nor SDK currently declares a secure-storage dependency or platform-backed implementation. Move app auth tokens and SDK signed session identity to the SDK core `SecureKeyValueStore` abstraction in a separate low-risk pass after dependency/platform decisions.
