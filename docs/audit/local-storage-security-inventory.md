...
- Secure storage migration: neither app nor SDK currently declares a secure-storage dependency. Move app auth tokens and SDK signed session identity to an existing or newly approved secure-storage abstraction in a separate low-risk pass.
